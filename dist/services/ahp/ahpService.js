...
export function rankUniversitas(criteriaWeights, userLat, userLong) {
    return __awaiter(this, void 0, void 0, function* () {
        const ahp = new AHP();
        const data = yield getParsedUniversitas();
        const filtered = data.filter(d => d.tuition_fee !== null &&
        ...
        const parsed = filtered.map(d => {
            const distance = getDistance({ latitude: userLat, longitude: userLong }, { latitude: d.latitude, longitude: d.longitude });
            return {
                name: d.name,
        ...
        const normalized = parsed.map(p => (Object.assign(Object.assign({}, p), { accreditation: p.accreditation, biaya: (maxBiaya - p.tuition_fee) / (maxBiaya - minBiaya), passRate: (p.pass_percentage / maxRate), 
            // distanceScore: (maxDistance - p.distance) / (maxDistance - minDistance),
            distanceScore: getDistanceScore(p.distance), major_count: Math.sqrt(p.major_count) / Math.sqrt(maxMajorCount), acceptance_rate: Math.log(1 / (p.acceptance_rate / 100)) / acceptanceMaxVal })));
        ...
        function akreditasiToScore(akreditasi) {
        ...
        function createTriplets(criterion) {
            const triplets = [];
            const biayaGabungan = [];
            const debugData = [];
